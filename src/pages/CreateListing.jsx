...
//Declaring functional component 'CreateListing'
export default function CreateListing() {
  ...
  //Declaring and initializing state variables using hooks
  //geoLocationEnable set to true uses Google, False using user input
  ...
  //Destructuring form data from state variables
  const {
    type,
    name,
    bedrooms,
  ...
  //Event handler for changing the input fields' values
  function onChange(e) {
    let boolean = null;
    ...
    //If files are selected, then store them in state
    if (e.target.files) {
    ...
    //If input type is text/boolean/number, then update its value in state
    if (!e.target.files) {
      setFormData((prevState) => ({
        ...prevState,
  ...
  async function onSubmit(e) {
    ...
    //If geolocation is enabled, get the latitude and longitude of the input address using Google Maps API
    let geolocation = {};
    let location;
  ...
  //If form is being submitted and loading is true, then display the spinner
  if (loading) {
    return <Spinner />;
  }
      ...
      <form onSubmit={onSubmit}>
        ...
        <div className='flex'>
          <button
            ...
            text-sm uppercase shadow-md rounded hover:shadow-lg focus:shadow-lg 
        ...
        <button
          type='submit'
          className='mb-6 w-full px-7 py-3 bg-blue-600 text-white font-medium text-sm uppercase rounded shadow-md hover:bg-blue-700 hover:shadow-lg focus:bg-blue-700 focus:shadow-lg active:bg-blue-800 active:shadow-lg transition duration-150 ease-in-out'>Create Listing</button>
      </form>
    </main>
  );
}
